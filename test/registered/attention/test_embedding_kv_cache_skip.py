...
class FakePool:
    ...
    def __init__(self, max_total_tokens, num_kv_heads, qk_head_dim, v_head_dim, dtype):
        ...
        self.k_buffer = torch.zeros(max_total_tokens, num_kv_heads, qk_head_dim, dtype=dtype)
        self.v_buffer = torch.zeros(max_total_tokens, num_kv_heads, v_head_dim, dtype=dtype)
...
def build_batch(seq_lens_list, num_kv_heads, qk_head_dim, v_head_dim, dtype,
    ...
    pool = FakePool(
        ...
        num_kv_heads, qk_head_dim, v_head_dim, dtype,
    )
    ...
    for i, (pl, sl) in enumerate(zip(prefix_lens_list, seq_lens_list)):
        if pl > 0:
            prefix_loc = torch.arange(prefix_offset, prefix_offset + pl, dtype=torch.long)
    ...
    fb = FakeForwardBatch(
        extend_prefix_lens, extend_seq_lens, seq_lens, orig_seq_lens,
...
def _run_one(backend, seq_lens_list, q, k, v, layer, dtype, prefix_lens_list):
    pool, req_to_token, fb, _ = build_batch(
        seq_lens_list, layer.tp_k_head_num, layer.qk_head_dim, layer.v_head_dim,
        dtype, prefix_lens_list=prefix_lens_list,
    )
...
def compare_paths(
    seq_lens_list, tp_q_head_num=16, tp_k_head_num=8, tp_v_head_num=8,
    qk_head_dim=128, v_head_dim=128, dtype=torch.float32, prefix_lens_list=None,
    sliding_window_size=-1, skip_enabled=True,
):
    ...
    layer = FakeLayer(
        tp_q_head_num, tp_k_head_num, tp_v_head_num, qk_head_dim, v_head_dim,
        scaling=1.0 / (qk_head_dim ** 0.5), sliding_window_size=sliding_window_size,
    )

    backend_orig = make_backend(skip_enabled=False)  # always original path
    out_orig = _run_one(backend_orig, seq_lens_list, q, k, v, layer, dtype, prefix_lens_list)

    backend_skip = make_backend(skip_enabled=skip_enabled)
    out_skip = _run_one(backend_skip, seq_lens_list, q, k, v, layer, dtype, prefix_lens_list)
